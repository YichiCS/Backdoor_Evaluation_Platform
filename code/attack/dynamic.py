...
class poison_generator():
    ...
    def __init__(self, ckpt_path, channel_init, steps, input_channel, normalizer, denormalizer, dataset, poison_rate, path, target_class=0, cuda_devices='0'):
        ...
        netG = Generator(channel_init=channel_init, steps=steps, input_channel=input_channel, normalizer=normalizer, denormalizer=denormalizer)
        ...
        netM = Generator(channel_init=channel_init, steps=steps, input_channel=input_channel, normalizer=normalizer, denormalizer=denormalizer, out_channels=1)
    ...
    def generate_poisoned_set(self):     
        ...
        for i in range(self.num_img):
            ...
            if pt < num_poison and poison_indices[pt] == i:
                ...
                mask_output = self.mask_generator.threshold(self.mask_generator(inputs))

                bd_inputs = inputs + (pattern - inputs) * mask_output
                img = bd_inputs.detach().squeeze().cpu()
                pt += 1

                os.makedirs(os.path.join(self.path, 'poisoned_img'), exist_ok=True)
                save_image(img if self.denormalizer is None else self.denormalizer(img), os.path.join(self.path, 'poisoned_img', f'label_{i}.png'))
...
class poison_transform():
    def __init__(self, ckpt_path, channel_init, steps, input_channel, normalizer, denormalizer, target_class=0, has_normalized=False, require_normalization=True):

        self.has_normlized = has_normalized
    ...
    def transform(self, data, labels):
        ...
        if (not self.has_normlized) and (self.require_normalization):
            data = self.normalizer(data)
        elif (self.has_normlized) and (not self.require_normalization):
...
"""
Generator from official implementation
"""
